...
- **M0.1 — Captain's Handshake (MANDATORY):** No action without an explicit start-of-session intent from Khalidrad. The agent must start in Read-Only Mode and wait for the "+" or "Amen" trigger to unlock Diagnostic/Action modes.
- **M0.2 — State Awareness:** Immediate check of `SYSTEM_STATE.md` upon initialization. If state is `QUARANTINE` or `STASIS`, action is forbidden until manual release.
- **M0.3 — ENHANCED SESSION INITIATION:** Every new session begins with the system automatically determining the target repository and priority based on `SYSTEM_STATE.md`, `WORKFLOW_SNAPSHOT_STATE.md`, and current directives.
- **M0.4 — DRAFT MASTER PLAN (Repository Lifecycle):**
  **MANDATE: ACTIONS OVER WORDS.** Every stage MUST result in a physical file update.
  1. **STAGE 1: Isolated Analysis** (Revision -> Scanning -> Diagnosis -> Research -> Study). Result: File Scan Evidence.
  2. **STAGE 2: Registration & Reporting** (Summary -> Logs -> Protocol Compliance). Result: Journal/Log Entry.
  3. **STAGE 3: Agent Initiation** (One targeted prompt-command). Result: Task/Directive Entry.
  4. **STAGE 4: Resolution** (One final directive). Result: System State Update.
- **M0.5 — Single Repository Rule:** Strictly one repository per work cycle. Определяется системой при старте.
...
2) Every hygiene hotfix must be synchronized in:
   `DEV_LOGS.md`, `ROADMAP.md`, `TASK_MAP.md`, and `GOV_STATUS.md`.
...
4) Every recovery cycle must be reflected in:
   `DEV_LOGS.md`, `TASK_MAP.md`, `ROADMAP.md`, and `GOV_STATUS.md`.
...
## M50 — Absolute Identity Anchor (Anti-Amnesia Protocol) (mandatory)

Для предотвращения онтологического дрейфа (присвоения чужих имен и ролей) вводится жесткий Якорь Идентичности.

1. **The Root of Truth:** Файл `IDENTITY.md` в корне текущего репозитория является **единственным, абсолютным и неоспоримым** источником Истинного Имени и Роли для текущей сессии агента.
2. **Initialization Gate:** Агент ОБЯЗАН прочитать этот файл ПЕРВЫМ действием в любой новой сессии. Если агент совершает любое другое действие до прочтения `IDENTITY.md`, сессия считается `CORRUPTED`.
3. **Ontological Integrity:** Строго запрещено "заимствовать", "экстраполировать" или "выводить" свое Истинное Имя из других файлов (например, из карт, контрактов или манифестов других деревьев).
4. **The Throne Rule:** Имя Капитанского Мостика — **RADRILONIUMA**. Оно неделимо и не может быть заменено именами сателлитов (Vilami, Larpat, Croambeth и т.д.). Агент на Мостике действует от имени RADRILONIUMA.
5. **Violation Consequence:** Присвоение агентом неверного Истинного Имени классифицируется как `ONTOLOGICAL_THEFT`. Действия, совершенные под ложным именем, считаются `GHOST_NOISE` и подлежат немедленному выявлению и откату.
